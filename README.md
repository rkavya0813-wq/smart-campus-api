...
## Overview
This coursework project implements a simple REST API for a Smart Campus. It exposes rooms, sensors, and sensor readings using plain JAX-RS (Jersey) with in-memory storage, validation, and consistent JSON error responses. It also includes structured error handling and request/response logging for robustness. All data is stored in-memory using Java collections (HashMap and ArrayList), with no database used.
...
11. Delete Room With Assigned Sensors (409)

This 409 example applies when room R101 still has sensors linked to it.
...
### Question 4
Yes, the DELETE operation in this implementation is idempotent, as there is no change in the final state of the system due to repeating the same request. As an example, when a client sends a DELETE request to /rooms/R102 to empty a room, the initial request will delete the room and give a 204 No Content. When another request is made with the same request, the API responds with 404 Not Found due to the room no longer existing. The result of the response is different, but the ultimate system state is the same after the initial successful deletion.
...
This behavior is evident in the DELETE /rooms/{roomId} endpoint within this implementation. It should also be noted that if a room still has linked sensors, the API prevents deletion and returns a 409 Conflict response.
...
Consequently, it will fail to reach the method logic, and the server will send back a 415 Unsupported Media Type response. This mechanism creates uniformity of the data format and guarantees that only valid and expected input is processed by the API.
This is enforced in the POST endpoints of this API, which accept only JSON requests.
In my final implementation, this behavior is explicitly handled by a NotSupportedExceptionMapper, which returns a 415 Unsupported Media Type response with a structured JSON error body.
...
### Question 9
Allowing internal Java stack traces to be exposed to API consumers can be very dangerous in security terms. Stack traces can be sensitive, containing class names, package structure, file paths, method names and line numbers. This information may provide attackers with information on how the system is internally organized.
...
In my implementation, this is handled using a GlobalExceptionMapper, which intercepts unexpected errors and returns a generic 500 Internal Server Error response without exposing internal details.

### Question 10
Applying JAX-RS filters to cross-cutting concerns, like logging, is beneficial since the functionality is concentrated in a single location and is applicable to various application areas. In this implementation, a LoggingFilter is used to log incoming requests and outgoing responses across all endpoints.
